...
def PBC_apdl_commands(PairedNodes,StrainState,Normal,EntityMasterName,EntityType):
    ...
    if EntityType == 'vertice':
        ...
        for j in range(0,2): #Only put the P.B.C on plane
...
class PeriodicCommandSetup():
    ...
    def __init__(self,geomacroname='Default',geomacro_folder='Default',tol=0.0001):
        ...
        if geomacro_folder == 'Default':
            ...
            if not CheckIfFolderExists(geomacro_folder):
                ...
                input("Press enter on terminal to continue...")
    ...
    def Write_Macro(self,Solve_macroname='macro_to_solve',Solvemacrofolder='Default',StrainStateValue=1,StrainCase='ALL',directory_to_save='Default'):
        ...
        def BeggingCommands(f=f,GeoMacroInput = self.macroname, path = self.geomacro_folder, StrainCase=StrainCase):
            '''
            ...
            f.write("\n!                                        !")
            f.write("\n!----------------------------------------!")
            f.write("\n!  ANALYSIS FOR {} STRAIN STATE          ".format(StrainCase))
            f.write("\n!Clearing All Data:")
            f.write("\n/CLEAR,START")
            f.write("\n/PREP7")
        ...
        def Write_BC_commands(StrainCase):
            '''
            ...
            def Write_UDBC(f,StrainCase):
                '''
                Method to create UDBC only for the z degree of freedom of boundary
                nodes
                '''
                # f = open("UDBC.temp",'w')
                ...
                CreateBC(df=self.nodedataframe,StrainCase=StrainCase,StrainValue=StrainStateValue)
                
                #Adding the Node Components Commands
                AddExternalDataInFile(f,"BC_components.temp")

                #Adding the Displacements Function Commands
                AddExternalDataInFile(f,"functionwriting.temp")
                
                #Adding the Discplacements Commands
                AddExternalDataInFile(f,"BC_creation.temp")
            ...
            for EachEntity in self.EntMastersNames:
                ...
                f_ce.write(data)
                #Writing the commands for out-of-plane displacement
            
            Write_UDBC(f=f_ce,StrainCase=StrainCase)
